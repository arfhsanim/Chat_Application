...
import com.labproject.chatapp.network.ApiClient;
import com.labproject.chatapp.network.ApiService;
...
public class ChatActivity extends BaseActivity {
    ...
    private void sendMessage() {
        ...
        message.put(Constants.KEY_MASSAGE, binding.inputMessage.getText().toString());
        ...
        if (!isReceiverAvailable){
            try{
                ...
                data.put(Constants.KEY_MASSAGE, binding.inputMessage.getText().toString());
        ...
        database.collection(Constants.KEY_COLLECTION_USERS).document(
            ...
            if (value != null) {
                ...
                if (receiverUser.image == null){
                    ...
                    chatAdapter.notifyItemRangeChanged(0, chatMassages.size());
                }
        ...
        if (value != null) {
            ...
            for (DocumentChange documentChange : value.getDocumentChanges()) {
                if (documentChange.getType() == DocumentChange.Type.ADDED) {
                    ChatMassage chatMessage = new ChatMassage();
                    chatMessage.senderId = documentChange.getDocument().getString(Constants.KEY_SENDER_ID);
                    chatMessage.receiverId = documentChange.getDocument().getString(Constants.KEY_RECEIVER_ID);
                    chatMessage.massage = documentChange.getDocument().getString(Constants.KEY_MASSAGE);
                    chatMessage.dateTime = getReadableDateTime(documentChange.getDocument().getDate(Constants.KEY_TIMESTAMP));
                    chatMessage.dateObject = documentChange.getDocument().getDate(Constants.KEY_TIMESTAMP);
                    chatMassages.add(chatMessage);
                }
            }
            ...
            if (count == 0) {
                ...
                binding.chatRecyclerView.smoothScrollToPosition(chatMassages.size() - 1);
            }
